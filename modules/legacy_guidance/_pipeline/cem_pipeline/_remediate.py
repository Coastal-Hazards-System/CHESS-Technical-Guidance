...
Run from modules/legacy_guidance/manuals/cem:  python ../../_pipeline/cem_pipeline/_remediate.py
"""
import os
import re
import sys
...
def fix_equations(text):
    ...
    text = notation.fix_merges(text)        # specific line-break word merges
    return text


def main():
    tot_prose = tot_eq = tot_hdr = 0
    for md in sorted(glob.glob("markdown/**/*.md", recursive=True)):
        ...
        hdr = notation.fix_headers(md)         # strip leaked running headers / page footers
        tot_prose += prose
        tot_eq += n_eq
        tot_hdr += hdr
        if prose or n_eq or nav or deflists or subsup or hdr:
            print(f"{md}: prose={prose} eq={n_eq} nav={nav} deflists={deflists} subsup={subsup} hdr={hdr}")
    ...
    print(f"TOTAL prose patterns={tot_prose}, equation fixes={tot_eq}, header/footer lines removed={tot_hdr}")
