...
```math
\frac{q}{\sqrt{gH_s^3}} = \begin{cases} 5(10^{-2}) \\ 1(10^{-2}) \end{cases} 0.0199 \approx 2(10^{-2}) \text{ (mean)}
```
...
\begin{cases} 
0.344 & 3.7 \\
0.136 \rightarrow \text{mean, m}^3/\text{s per meter} & 1.46 \rightarrow \text{mean, ft}^3/\text{s per foot} \\
0.069 & 0.74 
\end{cases}
```
...
```math
H_b = \Delta D_{n50} \left( K_D \cot \alpha \right)^{1/3} = \left( \frac{\rho_s}{\rho_w} - 1 \right) (4.94 \text{ m}) (3.5 \times 2.0)^{1/3}
```
...
```math
H_b = 1.57(4.94)(2.0 \times 2.0)^{1/3} = 12.3\text{ ft (3.76 m)}
```
...
```math
\xi_{mc} = \left[ 6.2 P^{0.31} (\tan \alpha)^{0.5} \right]^{1/(P+0.5)}
```
...
```math
\frac{f_i H_s}{\Delta D_{n50}} = 1.0 \text{ S}^{-0.2} P^{-0.13} N_z^{-0.1} \left(\cot \alpha\right)^{0.5} \xi_m^P \qquad \text{surging } (\xi_m > \xi_{mc})
```
...
All of the preceding results are for $N_{z}$ = 2,500 waves and for $T_{p}$ = 14 s which is roughly, Tm = 11.6 and t = 8-hr storms. It is instructive to consider increased storm durations shown as follows. Armor weights are in pounds
(Sheet 3 of 7)
...
```math
h' = h^* + s = 20\text{ ft} + 8.1\text{ ft} = 28.1\text{ ft}
```
