...
- (b) To overcome these problems, a transformation of the basic variables \overline{X} = (X_1, X_2, ..., X_n) into a new set of normalized variables \overline{Z} = (Z_1, Z_2, ..., Z_n) is performed. For uncorrelated normally distributed basic variables \overline{X} the transformation is
...
```math
\sigma' = \frac{\sigma}{\mu} = \frac{\text{standard deviation}}{\text{mean value}} \tag{VI-6-37}
```
...
```math
\hat{\zeta}_p = (\cot\hat{\alpha})^{-1} \ (\hat{s}_{op})^{-0.5}
```
