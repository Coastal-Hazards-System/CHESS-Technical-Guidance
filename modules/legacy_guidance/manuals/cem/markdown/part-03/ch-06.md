...
- Figure III-6-5. Comparison of current profile in the presence of waves predicted by present model
- Figure III-6-6. Sketch of turbulent flow with logarithmic profile over a granular bed
- Figure III-6-7. Shields diagram for initiation of motion in steady turbulent flow (from Raudkivi (1976))
- Figure III-6-8. Modified Shields diagram (Madsen and Grant 1976)
- Figure III-6-9. Comparison of Shields curve with data on initiation of motion in oscillatory turbulent flows (Madsen and Grant 1976)
- Figure III-6-10. Conceptualization of pressure drag $τ_{b}O,$ skin friction $τ_{b}N,$ total shear stress $τ_{b}N+τ_{b}O,$ for turbulent flow over a rippled bed
...
```math
\tan \varphi = 0.242 \rightarrow \varphi = 13.6^{\circ} \approx 14^{\circ}
```
...
```math
f_{cw}/C_{\mu} \approx 0.053 \rightarrow f_{cw} \approx 0.062
```
...
```math
\tan \varphi = 0.79 \rightarrow \varphi = 38^{\circ}
```
...
```math
\frac{\eta}{\lambda} = 1.05 \times 10^{-2} Z^{-0.65} = 6.91 \times 10^{-2} \rightarrow \lambda = \frac{\eta}{(\eta/\lambda)} = 15.9 \text{ cm} \approx 16 \text{ cm}
```
...
```math
tan \; \phi' = 0.25 \; {\scriptstyle \rightarrow} \; \phi' = 13.8^\circ \; \approx \; 14^\circ
```
...
```math
\tan \, \phi_s = 0.77 \, \rightarrow \, \phi_s = 37.6^{\,\circ} \, \approx \, 38^{\,\circ}
```
...
| τNb | Bottom skin friction shear stress $[force/length^{2}]$ |
