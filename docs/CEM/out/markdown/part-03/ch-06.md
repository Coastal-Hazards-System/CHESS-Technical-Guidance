...
(8) For rough turbulent flows $z_{0}$ = $k_{n}/30$ and Equation 6-8 is an explicit equation for $f_{c}$ in terms of the relative roughness $z_{r}/k_{n}.$ For smooth turbulent flows z 0 = $ν/(9u_{*c})$ and Equation 6-8 leads to an implicit equation for $f_{c}$ (Madsen 1993)
...
(d) The steady flow is entirely in the direction of the current, i.e., at an angle φ wc to the wave direction. Invoking the "law of the wall," the current velocity is (for z < $δ_{cw}$ where $δ_{cw}$ is defined in Equation 6-38)
...
- 3) If zr = 7 D < $δ_{cw}/π,$ steps 3 through 5 of Example Problem III-6-12 are followed.
...
- 7) If zr = 7 D < $z_{0}$ = kn /30, the mean wave-associated suspended sediment transport is obtained q ¯ sSw from Equations 6-96 through 6-98.
...
obtained from Example Problems III-6-5 and 8. The sediment suspension phase angle φs is obtained from Equation 6-83 with zr = 7 D = 0.14 cm
...
Since $z_{r}$ = 0.14 cm < z 0 = 0.15 cm, the wave-associated mean transport is obtained from Equation 6-96.
...
- (c) For each size class, the suspended load transport is now computed as detailed in Part III-6(5)e using the fall velocity $w_{f,n}$ appropriate for the nth size class.
